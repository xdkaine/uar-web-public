...
describe('mass email content rendering', () => {
  ...
  it('appends the portal footer to HTML and text output', () => {
    ...
    expect(content.html).toContain('You are receiving this email because your account was selected by an administrator.');
    expect(content.html).toContain('data-mass-email-footer="true"');
    expect(content.text).toContain('Maintenance tonight.');
    expect(content.text).toContain('selected by an administrator');
    expect(content.text).toContain('Cal Poly SOC');
  ...
  it('does not duplicate the portal footer when rendered campaign HTML is saved again', () => {
    ...
    expect(secondRender.text.match(/selected by an administrator/g)).toHaveLength(1);
  });
});